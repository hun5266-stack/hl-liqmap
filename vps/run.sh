...
# 돌고 있는 스크립트를 덮어쓰면 bash 가 중간부터 잘못 읽으므로
...
MSG="snapshot $(date -u +%Y-%m-%dT%H:%MZ) [vps]"
git commit -q -m "$MSG"

for i in 1 2 3; do
  if git push -q origin main; then echo "푸시 완료"; exit 0; fi
  echo "푸시 거부 — 재정렬 $i"
  T=$(mktemp -d)
